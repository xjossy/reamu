...
class DescribingPage extends StatefulWidget {
  const DescribingPage({super.key});
...
class _DescribingPageState extends State<DescribingPage> {
  ...
  void _showSummarization() async {
    ...
    Navigator.push(
      ...
      // Check if we need to move to next note or repeat current note
      final hasMoreNotes = await _settingsService.hasMoreNotes(_userProgress);
      ...
      if (hasMoreNotes) {
        ...
        // Reset for new session with same note
        setState(() {
          _currentQuestionIndex = 0;
          _selectedOption = null;
          _answers.clear();
